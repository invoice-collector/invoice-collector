
import { SketchCollector } from '../../sketchCollector';
import { CollectorCaptcha, CollectorState, CollectorType, CollectorAuthenticationMethod } from '../../abstractCollector';

export class UpsAccessPointCollector extends SketchCollector {

    static CONFIG = {
        id: 'ups_access_point',
        name: 'UPS Access Point',
        description: 'i18n.collectors.ups_access_point.description',
        version: '0',
        website: 'https://ap.ups.com/REAP/viewAccessPoint.htm',
        logo: 'https://portal-ui-images.s3.eu-central-1.amazonaws.com/logo/120x120/1274279.jpg',
        type: CollectorType.SKETCH,
        params: {
            email: {
                type: 'email',
                name: 'i18n.collectors.all.email',
                placeholder: 'i18n.collectors.all.email.placeholder',
                mandatory: true,
            },
            password: {
                type: 'password',
                name: 'i18n.collectors.all.password',
                placeholder: 'i18n.collectors.all.password.placeholder',
                mandatory: true,
            },
        },
        loginUrl: 'https://ap.ups.com/REAP/viewAccessPoint.htm',
        captcha: CollectorCaptcha.NONE,
        authenticationMethod: CollectorAuthenticationMethod.ALL,
        state: CollectorState.PLANNED,
    };

    /**
     * Constructs a new instance of the UpsAccessPointCollector class.
     */
    constructor() {
        super(UpsAccessPointCollector.CONFIG);
    }
}
